...
public class CameraDetectView extends JavaCameraView implements PictureCallback {
    ...
    public List<Size> getSelectedResolutionList() {
        ...
        while (i < selectedSupportedResolutions.size()) {
            if (((double) ((Size) selectedSupportedResolutions.get(i)).width) / 10.0d != ((double) (((Size) selectedSupportedResolutions.get(i)).width / 10)) || ((double) ((Size) selectedSupportedResolutions.get(i)).height) / 10.0d != ((double) (((Size) selectedSupportedResolutions.get(i)).height / 10))) {
    ...
    public void setInitialResolution() {
        ...
        for (i = 0; i < selectedSupportedResolutions.size(); i++) {
            if (selectedResolutionDist > Math.abs(((Size) selectedSupportedResolutions.get(i)).width - 640)) {
                selectedResolutionDist = Math.abs(((Size) selectedSupportedResolutions.get(i)).width - 640);
                selectedResolutionIndex = i;
            }
        }

        Size selectedResolution = (Size) selectedSupportedResolutions.get(selectedResolutionIndex);
