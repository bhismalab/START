...
public class TestData {
    ...
    public static final String FILED_ITEMS = "questions";
    public static final String FILED_ITEMS_VIDEO = "videoQuestions";
    ...
    @SerializedName(FILED_PARENT)
    private String mParent = null;

    @SerializedName(FILED_ITEMS)
    private ArrayList<TestDataItem> mItems = null;

    @SerializedName(FILED_ITEMS_VIDEO)
    private ArrayList<TestDataVideoItem> mVideoItems = null;
    ...
    public ArrayList<TestDataVideoItem> getVideoItems() {
        return mVideoItems;
    }

    public void setVideoItems(ArrayList<TestDataVideoItem> videoItems) {
        mVideoItems = videoItems;
    }

    public int getTotalCountItems() {
        return mItems.size() + mVideoItems.size();
    }

    public int getAnsweredCountItems() {
        int result = 0;

        for (TestDataItem item : mItems) {
            if (item.getSelectedChoice() != -1) {
                result++;
            }
        }

        for (TestDataVideoItem item : mVideoItems) {
            if (item.getSelectedChoice() != -1) {
                result++;
            }
        }
