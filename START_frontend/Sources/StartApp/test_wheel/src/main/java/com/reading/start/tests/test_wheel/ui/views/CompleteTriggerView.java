package com.reading.start.tests.test_wheel.ui.views;
...
public class CompleteTriggerView extends View {
    ...
    private boolean onBottomLeftTap() {
        ...
        if (!mBottomLeftTap && mTopRightTap && mTopLeftTap && Math.abs(Calendar.getInstance().getTimeInMillis() - mLastTapTime) < 2000) {
            mBottomLeftTap = true;
            mLastTapTime = Calendar.getInstance().getTimeInMillis();
            result = true;
        ...
        } else if (mBottomLeftTap && mTopRightTap && mTopLeftTap && Math.abs(Calendar.getInstance().getTimeInMillis() - mLastTapTime) < 2000) {
            result = true;
        }
