...
public class CameraLog {
    private static final String TAG = CameraLog.class.getSimpleName();
    ...
    private static void init() {
        try {
            ...
            File folder = new File(root + "/" + Constants.CAMERA_LOG_FOLDER);

            if (!folder.exists()) {
                folder.mkdir();
            }

            File logFile = new File(folder, Constants.CAMERA_LOG_FILE);
