...
import com.reading.start.tests.test_parent_child_play.BuildConfig;
...
import com.reading.start.tests.test_parent_child_play.databinding.TestParentChildPlayFragmentTestBinding;
...
import com.reading.start.tests.test_parent_child_play.ui.dialogs.DialogCompleteTest;
import com.reading.start.tests.test_parent_child_play.ui.views.AutoFitTextureView;
import com.reading.start.tests.test_parent_child_play.utils.Utility;
...
public class ParentChildPlayTestFragment extends BaseFragment {
    private static final String TAG = ParentChildPlayTestFragment.class.getSimpleName();
    ...
    private TestParentChildPlayFragmentTestBinding mBinding;
    ...
    public ParentChildPlayTestFragment() {
        // Required empty public constructor
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mBinding = DataBindingUtil.inflate(inflater, R.layout.test_parent_child_play_fragment_test, container, false);
    ...
    @Override
    public void onPause() {
        if (mInProgress && mIsRecordingVideo) {
            stopRecordingVideo();
            onTestCompleted();
        }
    ...
    private void showEndTestDialog() {
        ...
        if (activity != null) {
            ...
            boolean showNextStep = !TestsProvider.getInstance(getActivity()).isLastTest(ParentChildPlayTest.getInstance());

            String title = mTestData.isInterrupted() ? getResources().getString(R.string.test_parent_child_play_dialog_interrupted_title)
                    : getResources().getString(R.string.test_parent_child_play_dialog_success_title);
            String message = mTestData.isInterrupted() ? getResources().getString(R.string.test_parent_child_play_dialog_interrupted_message)
                    : getResources().getString(R.string.test_parent_child_play_dialog_success_message);
            ...
            if (!mSaving) {
                ...
                if (activity != null) {
                    if (realm != null && !realm.isClosed()) {
                        ...
                        ParentChildPlayTestSurveyResult result = new ParentChildPlayTestSurveyResult();
                        result.setSurveyId(activity.getSurveyId());
                        Gson gson = new Gson();
                        String testValue = gson.toJson(mTestData);
                        result.setResultFiles(testValue);
                        result.setTestRefId(ParentChildPlayTest.TYPE);
                        result.setStartTime(mTestData.getStartTime());
                        result.setEndTime(mTestData.getEndTime());

                        try {
                            realm.beginTransaction();
                            Number currentId = realm.where(ParentChildPlayTestSurveyResult.class).max(ParentChildPlayTestSurveyResult.FILED_ID);
                            int nextId;
                            ...
                            attempt = (int) realm.where(ParentChildPlayTestSurveyResult.class)
                                    .equalTo(ParentChildPlayTestSurveyResult.FILED_SURVEY_ID, activity.getSurveyId()).count();
                        } catch (Exception e) {
                            TestLog.e(TAG, e);
                ...
                mCameraDevice.createCaptureSession(surfaces, new CameraCaptureSession.StateCallback() {
                    ...
                    public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                        ...
                        getActivity().runOnUiThread(() -> {
                            // UI
//                        mBinding.buttonSave.setText(R.string.test_parent_child_play_button_save);
