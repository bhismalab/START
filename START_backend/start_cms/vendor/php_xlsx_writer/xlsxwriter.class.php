...
class XLSXWriter {
	...
	public function writeSheetRow($sheet_name, array $row) {
		...
		foreach ($row as $k => $v) {
			$format = 'string';
			if (isset($sheet->columns[$column_count])) {
				$format = $sheet->columns[$column_count];
			}
			$this->writeCell($sheet->file_writer, $sheet->row_count, $column_count, $v, $format);
			$column_count++;
		}
		$sheet->file_writer->write('</row>');
		$sheet->row_count++;
	...
	protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $cell_format_index) {
		$cell_type = 'string';
		if (!empty($this->cell_types[$cell_format_index])) {
