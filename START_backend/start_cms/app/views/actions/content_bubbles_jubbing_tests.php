				...
				<div class="row">
					<div class="col-md-6 col-md-offset-3">
                        <div class="form-group">
                            ...
                            <?php if (!empty($exercise['bubble_img'])) {
                                $uploadUrl = SITE.utils::dirCanonicalPath(CMS_DIR.UPLOADS_DIR);
                                $previewUrl = $uploadUrl.'tests_content/'.$exercise['bubble_img'];
                                $preview_exists = is_file(UPLOADS_DIR.'tests_content/'.$exercise['bubble_img']);
                                //var_dump($preview_exists); die("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAa");
                                ?>
                                <div class="image-preview">
                                    <img src="<?=($preview_exists? $previewUrl: IMAGE_DIR.'noimg.jpg');?>" alt="<?=$exercise['bubble_img'];?>" class="img-responsive image-preview-img" />
                                    <div class="image-preview-info">
                                        <p>
                                            <?php
                                            $orgSize = utils::getFileSizeFormatted(UPLOADS_DIR.'tests_content/'.$exercise['bubble_img']);

                                            $tmbSize = utils::getFileSizeFormatted(UPLOADS_DIR.'tests_content/'.$exercise['bubble_img']);
                                            $imgModTimestamp = @filemtime(UPLOADS_DIR.'tests_content/'.$exercise['bubble_img']);
                                            ?>
